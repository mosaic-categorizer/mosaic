...
from mosaic.classifier import (
    classify_trace,
    compute_file_temperatures_multiple,
    classify_multiple_temperatures,
)
...
from mosaic.visualizer import (
    visualize,
    generate_box_plots,
    generate_class_repartition_wrt_io,
    generate_all_distribution_plots,
    generate_all_occurrences_plots,
)
...
class Categorizer:
    ...
    def __init__(
        ...
        mount: str = "/",
        prune_executions: bool = True,
        duration_threshold: int = -1,
        rank: int = -1,
        n_ranks: int = -1,
    ):
        """
        ...
        if generate_graphs:
            Path(os.path.join(output_directory, "graphs")).mkdir(
        ...
        if os.path.isfile(os.path.join(trace_directory, "banned_traces.json")):
            ...
            if banned_traces["threshold"] == duration_threshold:
                ...
                pathlib.Path(
                    os.path.join(trace_directory, "banned_traces.json")
                ).unlink(missing_ok=True)
                pathlib.Path(os.path.join(trace_directory, "trace_hashes.json")).unlink(
                    missing_ok=True
                )
                pathlib.Path(
                    os.path.join(output_directory, "processed_traces.json")
                ).unlink(missing_ok=True)
        ...
        if prune_executions:
            if os.path.isfile(os.path.join(trace_directory, "trace_hashes.json")):
                print(
                    f'Restoring traces hashes from {os.path.join(trace_directory, "trace_hashes.json")}'
                )
                ...
                if os.path.isfile(
                    ...
                    print(
                        f'Restoring traces to process from {os.path.join(output_directory, "processed_traces.json")}'
                    )
                    with open(
                        os.path.join(output_directory, "processed_traces.json")
                    ) as f:
                ...
                process_pool.batch_submit(
                    traces, compute_trace_hash, self.trace_directory
                )
                process_pool.wait_completion()
                for res in process_pool.get_result():
                    trace, h = res
                    if h not in self.traces_of_hash:
            ...
            if not self.traces_to_process:
                ...
                self.traces_to_process = list(
                    random.choice(self.traces_of_hash[hg]) for hg in self.traces_of_hash
                )
                with open(
                    os.path.join(output_directory, "processed_traces.json"), "w"
                ) as f:
        ...
        self.traces_to_process = [
            trace for trace in self.traces_to_process if trace not in existing_results
        ]
        ...
        if rank != -1 and n_ranks != -1:
            ...
            print(
                f"Processing {len(self.traces_to_process)} traces on rank {rank} out of {n_ranks}"
            )
    ...
    def ban_executions_too_short(self, traces: list, threshold: int) -> list:
        ...
        process_pool = ProcessPool(os.cpu_count() - 1)
        process_pool.batch_submit(
            traces, exec_is_too_short, self.trace_directory, threshold
        )
        process_pool.wait_completion()
        traces_too_short = []
        for res in process_pool.get_result():
    ...
    def clean_traces(self) -> None:
        if not os.path.isfile(
            os.path.join(self.output_directory, "processed_traces.json")
        ):
    ...
    def generate_mongodb_export(self):
        ...
        for trace in traces_to_process:
            if not os.path.isfile(
                os.path.join(self.output_directory, f"{trace}.class.json")
            ):
                not_found += 1
                continue
            with open(os.path.join(self.output_directory, f"{trace}.class.json")) as f:
                try:
                    ...
                    print(f"Error decoding {trace}.class.json")
                    error += 1
                    continue
            if j["status"] == "error":
                error += 1
            ...
            if estimate:
                j["representation_count"] = len(
                    self.traces_of_hash[self.get_exec_hash(trace)]
                )
            j['infos']['file'] = trace.replace('.tef.json.gz', '')
            exported.append(j)
        with open(os.path.join(self.output_directory, "mongo_export.json"), "w") as f:
            json.dump(exported, f)
        print(
            f"Exported {len(exported)} results to mongo_export.json ({error} errors, {not_found} not found)"
        )
    ...
    def categorize_all_traces(
        ...
        process_pool = (
            ProcessPool(os.cpu_count() - 1) if n_proc == -1 else ProcessPool(n_proc)
        )
        process_pool.batch_submit(
            [
                os.path.join(self.trace_directory, trace)
        ...
        with tqdm(
            total=len(self.traces_to_process), file=sys.stdout, unit="traces"
        ) as pbar:
    ...
    def sort_traces(self, strategy: str) -> None:
        ...
        trace_sizes = {}
        for trace in self.traces_to_process:
            trace_sizes[trace] = os.stat(
                os.path.join(self.trace_directory, trace)
            ).st_size
        if strategy == "heaviest":
            self.traces_to_process = sorted(
                self.traces_to_process, key=trace_sizes.get, reverse=True
            )
            return
        if strategy == "lightest":
            self.traces_to_process = sorted(
                self.traces_to_process, key=trace_sizes.get, reverse=False
            )
            return
        raise NotImplementedError(f"{strategy} sort strategy not implemented")

    def generate_report_with_est_from_files(
        self, delete_on_error: bool = False
    ) -> None:
        results = []
        with open(os.path.join(self.output_directory, "processed_traces.json")) as f:
            traces_to_process = json.load(f)
        ...
        for p_trace in traces_to_process:
            if os.path.isfile(
                os.path.join(self.output_directory, p_trace + ".class.json")
            ):
                with open(
                    os.path.join(self.output_directory, p_trace + ".class.json")
                ) as f:
                    try:
                        ...
                        print(f"Error decoding {p_trace}.class.json")
                        continue
                    if j["status"] == "error":
                        ...
                        if delete_on_error:
                            os.remove(
                                os.path.join(
                                    self.output_directory, p_trace + ".class.json"
                                )
                            )
                    ...
                    total_io = sum(
                        [
                            i["data_operated_avg"] * i["segments_cnt"]
                            for i in (j["read"] + j["write"])
                        ]
                    )
                    results.append(
                        [
                            ...
                            [
                                class_list
                                for category in j["classes"].values()
                                for class_list in category
                            ],
                            total_io,
                        ]
                    )
        ...
        for t_ in hl:
            print(
                f"Temp {t_}s - mean {sum(mean_temp[t_]) / len(mean_temp[t_]):.2f} max {max(mean_temp[t_]):.2f}"
            )
        self.generate_report_with_est(results, len(traces_to_process), len(results))
        print(f"Result exported in {time.time() - t} seconds")

    def generate_report_with_est(
        self, results: list, n_selected: int, n_found: int
    ) -> None:
        """
        ...
        if estimate:
            estimated_categorized_all_traces = sum(
                map(
                    lambda t: len(self.traces_of_hash[self.get_exec_hash(t)]),
                    processed_traces,
                )
            )
        for class_ in classes:
            class_count_processed[f"{class_}_distribution"] = round(
                class_count_processed[class_] / categorized_traces, 3
            )
            if estimate:
        ...
        class_count_processed = dict(
            sorted(class_count_processed.items(), key=lambda x: x[0])
        )
        if estimate:
        ...
        with open(os.path.join(self.output_directory, "summary.json"), "w") as file:
            ...
            if hasattr(self, "traces_of_hash"):
                ...
                summary["infos"][
                    "inferred_executions"
                ] = estimated_categorized_all_traces
        ...
        if estimate:
            self.generate_heatmaps(
                traces_of_class, True, False, all_traces, "_estimated_all"
            )
        with open(
            os.path.join(self.output_directory, "traces_of_class.json"), "w"
        ) as file:
    ...
    def generate_heatmaps(
        ...
        estimate_all_traces: bool,
        gen_correlation: bool,
        all_traces: list,
        suffix: str = "",
    ) -> None:
        """
            ...
            corr = pd.DataFrame(
                {
                    cls: [
                        (
                            len(self.traces_of_hash[self.get_exec_hash(t)])
                            if t in trace_lst
                            else 0
                        )
                        for t in all_traces
        ...
        sns.heatmap(
            corr, annot=True, square=True, cmap="coolwarm", cbar_kws={"shrink": 0.82}
        )
        plt.savefig(
            os.path.join(self.output_directory, f"correlation_heatmap{suffix}.svg")
        )

    def compute_jaccard_sim(
        self, traces_of_class: dict, estimated: bool
    ) -> pd.DataFrame:
        """
        ...
        for cl1 in traces_of_class:
            values = []
            t1 = set(traces_of_class[cl1])
            t1_trace_count = len_of_cl[cl1]
            for cl2 in traces_of_class:
                t2 = set(traces_of_class[cl2])
                t2_trace_count = len_of_cl[cl2]
                intersection = set.intersection(t1, t2)
                if estimated:
                ...
                values.append(
                    intersection_trace_count
                    / (t1_trace_count + t2_trace_count - intersection_trace_count)
                )
            sim[cl1] = values
    ...
    def get_exec_hash(self, trace: str) -> str:
        ...
        if trace not in self.trace_hash_cache:
            _, self.trace_hash_cache[trace] = compute_trace_hash(
                trace, self.trace_directory
            )
    ...
    def recover_classifier_result(self, trace: str) -> list:
        ...
        return [
            class_list
            for category in result["classes"].values()
            for class_list in category
        ]
...
def compute_trace_hash(trace: str, trace_directory: str) -> (str, str):
    ...
    return (
        trace,
        f'{job["metadata"]["uid"]}_{job["metadata"]["exe"]}_{len(job["traceEvents"])}_{n_bytes}',
    )
...
def categorize_trace(
    ...
    output_graphs: bool,
    mount: str,
    metadata_spike_threshold: int = 10,
) -> list:
    """
    ...
    try:
        if os.path.isfile(
            os.path.join(output_directory, trace.split("/")[-1] + ".class.json")
        ):
            with open(
                os.path.join(output_directory, trace.split("/")[-1] + ".class.json"),
                "r",
                ...
                total_io = sum(
                    [
                        i["metadata_operations_avg"] * i["segments_cnt"]
                        for i in (j["read"] + j["write"])
                    ]
                )
            return [
        ...
        file_temperatures = compute_file_temperatures_multiple(
            job["traceEvents"], [1, 10, 60, 1800, 3600]
        )
        ...
        start_ts = time.time()
        clustering_write_segments = (
            find_periodic_patterns(job, "write", mount)
            if "write" in ftio_data and "write" not in ftio_res
            else []
        )
        clustering_read_segments = (
            find_periodic_patterns(job, "read", mount)
            if "read" in ftio_data and "read" not in ftio_res
            else []
        )
        ...
        result = {
            ...
            "classes": None,
            "metadata": metadata,
            "read": clustering_read_segments,
            "write": clustering_write_segments,
            "ftio": ftio_res,
        ...
        if output_graphs and (
            len(clustering_write_segments) > 0 or len(clustering_read_segments) > 0
        ):
            pass
            visualize(
                job,
                clustering_write_segments,
                classes["write_classes"],
                clustering_read_segments,
        ...
        with open(
            os.path.join(output_directory, trace.split("/")[-1] + ".class.json"), "w"
        ) as file:
    ...
    return [
        trace,
        [class_list for category in classes.values() for class_list in category],
    ]
