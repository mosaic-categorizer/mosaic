...
def clusterize(durations: list, amounts: np.array) -> list:
    ...
    abs_diff1 = not any(map(lambda x: (abs(x - mean1) / mean1) > 0.25, durations))
    mean2 = statistics.mean(amounts)
    abs_diff2 = not any(map(lambda x: (abs(x - mean2) / mean2) > 0.25, amounts))
    same_amounts = cv2 < 0.01 and abs_diff2
    if (
        cv1 < (0.1 if same_amounts else 0.01)
        and cv2 < 0.005
        and (same_amounts or (abs_diff1 and abs_diff2))
    ):
        return [0 for _ in range(len(durations))]
    norm_data_dur = np.array(durations).reshape(-1, 1)
    norm_data_amo = np.array(amounts).reshape(-1, 1)
    if cv1 < (0.1 if same_amounts else 0.01) and (same_amounts or abs_diff1):
        norm_data = norm_data_amo
    elif cv2 < 0.005 and abs_diff2:
        norm_data = norm_data_dur
    else:
        norm_data = np.concatenate((norm_data_dur, norm_data_amo), axis=1)
    mean_shift = MeanShift(
        bandwidth=max(estimate_bandwidth(norm_data, quantile=0.5), 0.1)
    )
...
def segment_characterization(operations_per_segment: dict, segments: list) -> dict:
    ...
    stats = {
        ...
        "duration_avg": (
            (segments[-1][0] - start_ts) / (len(segments) - 1)
            if len(segments) > 1
            else end_ts - start_ts
        ),
        "working_time_avg": statistics.mean(working_times),
        "working_time_cv": (
            statistics.stdev(working_times) / statistics.mean(working_times)
            if len(working_times) > 2
            else 0
        ),
        "data_operated_avg": statistics.mean(
            map(
                lambda op_l: sum(map(lambda op: op["args"]["count"], op_l)),
                operations_per_segment.values(),
            )
        ),
        "metadata_operations_avg": statistics.mean(
            map(
                lambda op_l: sum(map(lambda op: 2 * op["args"]["opens"], op_l)),
                operations_per_segment.values(),
            )
        )
        + statistics.mean(
            map(
                lambda op_l: sum(map(lambda op: op["args"]["seeks"], op_l)),
                operations_per_segment.values(),
            )
        ),
    }
...
def compute_activity_stats(
    segments: list, operations_per_segment: dict
) -> (list, list):
    """
...
def merge_neighbours(operations: list, total_seconds: int, avg_empty: float) -> None:
    ...
    while i < n_ops - 1:
        ...
        o2_s, o2_e = (
            operations[i + 1]["ts"],
            operations[i + 1]["ts"] + operations[i + 1]["dur"],
        )
        d = o2_s - o1_e
        dt = o2_e - o1_s
        if d < 0.001 * total_seconds or d < 0.5 * avg_empty or d / dt < 0.01:
            operations[i] = new_operation_from_merge(
                operations, i, min(o1_s, o2_s), max(o1_e, o2_e)
            )
            operations.pop(i + 1)
            n_ops -= 1
        else:
...
def new_operation_from_merge(operations: list, i: int, start_ts, end_ts) -> dict:
    ...
    new_op["args"]["ranks"] = (
        operations[i]["dur"] * operations[i]["args"]["ranks"]
        + operations[i + 1]["dur"] * operations[i + 1]["args"]["ranks"]
    ) / new_op["dur"]
    if new_op["args"]["file"] != operations[i + 1]["args"]["file"]:
        new_op["args"]["file"] = os.path.commonprefix(
            [new_op["args"]["file"], operations[i + 1]["args"]["file"]]
        )
    if (
        new_op["args"]["opens"] is not None
        and operations[i + 1]["args"]["opens"] is not None
    ):
        new_op["args"]["opens"] += operations[i + 1]["args"]["opens"]
    else:
        new_op["args"]["opens"] = None
    if (
        new_op["args"]["seeks"] is not None
        and operations[i + 1]["args"]["seeks"] is not None
    ):
...
def compute_metadata_stats(trace: dict, mount: str, spike_threshold: int) -> dict:
    ...
    operations = list(
        filter(
            lambda x: x["name"] in ["read", "write"]
            and x["args"]["opens"] is not None
            and x["args"]["seeks"] is not None
    ...
    for operation in operations:
        ...
        if timestamp_start in windows:
            windows[timestamp_start] += (
                operation["args"]["opens"] + operation["args"]["seeks"]
            )
        else:
            windows[timestamp_start] = (
                operation["args"]["opens"] + operation["args"]["seeks"]
    ...
    metadata_op_ps = (
        (sum(windows.values()) / (max(windows.keys()) - min(windows.keys())))
        if len(windows) > 1
        else 0
    )
    return {
...
def find_periodic_patterns(trace: dict, operation_type: str, mount: str) -> list:
    ...
    if empty_count > 0:
        merge_neighbours(
            operations,
            trace["metadata"]["end_ts"] - trace["metadata"]["start_ts"],
    ...
    classified_segments = []
    while segments:
        classified_segments += classify_one_segment_group(
            segments, operations_per_segments
        )
        operations_per_segments = {
            key: value
            for key, value in operations_per_segments.items()
            if key in segments
        }
    ...
    for i in range(len(operations) - 1):
        if operations[i + 1]["ts"] > operations[i]["ts"] + operations[i]["dur"]:
            ...
            total_empty_duration += (
                operations[i + 1]["ts"] - operations[i]["ts"] + operations[i]["dur"]
            )
...
def classify_one_segment_group(segments: list, operations_per_segments: dict) -> list:
    ...
    segments_amount = np.array(
        [
            sum(map(lambda v: v["args"]["count"], value))
            for key, value in operations_per_segments.items()
        ],
        dtype=np.float64,
    )
    ...
    segment_group = []
    cleaned_segments, cleaned_labels, cleaned_operations_per_segment = (
        copy(segments),
        copy(segment_duration_classes),
        copy(operations_per_segments),
    )
    for least_common_class in least_common_classes:
        segments_of_class = get_segments_of_label(
            cleaned_labels, cleaned_segments, least_common_class
        )
        segment_group += segments_of_class
        filtered_operation_per_segment = {
            key: value
            for key, value in cleaned_operations_per_segment.items()
            if key in segments_of_class
        }
        classified_segments.append(
            segment_characterization(filtered_operation_per_segment, segments_of_class)
        )
