...
def visualize(job: dict, pattern_write: list, write_classes: list, pattern_read: list,
    ...
    classification_scatter_write = create_categorizer_trace(pattern_write, job['metadata']['start_ts'],
                                                            job['metadata']['end_ts'], 'write')
    ...
    classification_scatter_read = create_categorizer_trace(pattern_read, job['metadata']['start_ts'],
                                                           job['metadata']['end_ts'], 'read')
...
def create_job_trace(job: dict, operation_type: str, mount: str) -> go.scatter:
    ...
    """
    accesses = list(
        filter(lambda x: x['name'] == operation_type and x['args']['file'].startswith(mount), job['traceEvents']))
...
def create_categorizer_trace(segments: list, start: float, end: float, operation_type: str) -> go.Scatter:
    """
    ...
    for segment in segments:
        for i in range(segment['segments_cnt']):
            ...
            x_ts.append(
                segment['start_ts'] + (i * segment['duration_avg'] + segment['working_time_avg']))
            x_ts.append(
                segment['start_ts'] + (i * segment['duration_avg'] + segment['working_time_avg']))
            y_amount.append(y_amount[-1])
            y_amount.append(y_amount[-1] - segment['n_ranks_avg'])
    ...
    return go.Scatter(x=x_ts, y=y_amount, mode='lines',
    ...
    for i in range(len(plots_fields)):
        ...
        for f in fields:
            if f not in sizes_per_class:
                continue
            violin = go.Violin(y=sizes_per_class[f], points=False, name=f, spanmode='hard', bandwidth=1e10)
...
def generate_donut_plot(title: str, plot_path: str, labels: list, values: list, inner_ring: list = None,
    ...
    labels = [l.replace('\\n', '\n') for l in labels]
    legend_labels = [f'{label} ({pct:.1f}%)' for label, pct in zip(labels, [v / total * 100 for v in values])]
    colors = [COLOR_MAPPING.get(get_label_discr(label), '#cccccc') for label in labels_old]
    plt.pie(values, radius=1, labels=legend_labels, colors=colors, labeldistance=1.05,
            wedgeprops={'linewidth': 2, 'edgecolor': 'white'})
    p = plt.gcf()
    p.gca().add_artist(my_circle)
    if inner_ring:
        plt.pie(inner_ring, radius=.49, colors=['white', palette.hex_colors[len(COLOR_MAPPING)]],
                wedgeprops={'linewidth': 2, 'edgecolor': 'white'})
        p.gca().add_artist(plt.Circle((0, 0), .4, color='white'))
        plt.figtext(0.5, 0.1, ring_text, ha="center", fontsize=8, color=palette.hex_colors[len(COLOR_MAPPING)])
    plt.text(0, 0, title, horizontalalignment='center', verticalalignment='center', fontsize=14, fontweight='bold')
...
def generate_stacked_plot(plot_path: str, labels: list, values: list) -> None:
    ...
    for i, (pct, label) in enumerate(zip(values, labels)):
        ...
        ax.barh(0, pct, left=left, color=COLOR_MAPPING.get(get_label_discr(label), '#cccccc'), height=0.1)
        if pct > 2.5:
            ax.text(left + pct / 2, 0, f"{pct:.0f}%", va='center', ha='center', fontsize=4.5, color='white')
        else:
            ax.text(left + pct / 2, 0.05, f"{pct:.0f}%", va='bottom', ha='center', fontsize=4.5, color='black')
    ...
    ax.legend(short_labels, loc='lower center', bbox_to_anchor=(0.5, .7),
              ncol=len(labels), fontsize=5.5, frameon=False, columnspacing=0.5, handletextpad=0.2)
...
def filter_values(labels: list, values: list, threshold: float = -1) -> (list, list):
    ...
    if threshold > -1:
        ordered = [val for val in ordered if (val[1] / sum_before_threshold) >= threshold]
    ...
    if sum_before_threshold - sum(filtered_values) > 0:
        ...
        filtered_values.insert(len(filtered_values), sum_before_threshold - sum(filtered_values))
...
def add_labels_to_color_mapping(labels: list) -> None:
    for label in labels:
        ...
        if label in COLOR_MAPPING:
            continue
        COLOR_MAPPING[label] = palette.hex_colors[min(len(COLOR_MAPPING), len(palette.hex_colors) - 1)]


def generate_all_distribution_plots(output_directory: str, from_est: bool = True) -> None:
    plots_fields = []
    ...
    for p in plots_fields:
        ...
        labels = []
        for c in p:
            if not c in classes_count:
                print(f'Class {c} is not found in summary.json file')
                continue
            labels.append(c)
            values.append(classes_count[c])
        print(p)
        filtered_labels, filtered_values = filter_values(labels, values, .025)
        add_labels_to_color_mapping(filtered_labels)
        generate_donut_plot(plot_title.replace(' ', '\n'),
                            os.path.join(output_directory, f'{plot_title.lower().replace(" ", "_")}.donut.svg'),
                            filtered_labels, filtered_values)
        generate_stacked_plot(os.path.join(output_directory, f'{plot_title.lower().replace(" ", "_")}.bar.svg'),
                              filtered_labels, filtered_values)


def generate_all_occurrences_plots(categorizer, traces_of_class: dict, estimate: bool) -> None:
    plots_fields = []
    generate_co_occurrence(categorizer, traces_of_class, 'all_classes', estimate=estimate)
    ...
    while len(plots_fields):
        ...
        generate_co_occurrence(categorizer, traces_of_class, f'cust_{i}', y_val, x_val, estimate=estimate)


def generate_co_occurrence(categorizer, traces_of_class: dict, name: str, y_classes: list = None,
                           x_classes: list = None, estimate: bool = True) -> None:
        ...
        for class_x in x_classes:
            ...
            if estimate:
                co_occurrence[class_y][class_x] = sum(
                    [len(categorizer.traces_of_hash[categorizer.get_exec_hash(t)]) for t in shared_objects]) / sum(
                    [len(categorizer.traces_of_hash[categorizer.get_exec_hash(t)]) for t in class_sets[class_y]])
            else:
                co_occurrence[class_y][class_x] = len(shared_objects) / len(class_sets[class_y])
    ...
    x_labels = [x.replace('read_', 'r_').replace('write_', 'w_').replace('metadata_', 'mt_') for x in x_classes]
    y_labels = [y.replace('read_', 'r_').replace('write_', 'w_').replace('metadata_', 'mt_') for y in y_classes]
    ...
    for i in range(len(y_classes)):
        for j in range(len(x_classes)):
            ax.text(j, i, f'{df_matrix.iloc[i, j]:.2f}', ha='center', va='center',
    ...
    print(x_labels)
    fig.set_size_inches(
        fig_width + max_ylabel_len + 0.1,
        fig_height + max_xlabel_len + 0.1
    )
    plt.subplots_adjust(left=0.2, right=0.9, bottom=0.2, top=0.9)
    plt.savefig(
        os.path.join(categorizer.output_directory, f'co_occurrences_heatmap_{name}.svg'),
        bbox_inches='tight')
    plt.close()
