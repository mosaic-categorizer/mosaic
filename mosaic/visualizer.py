...
def visualize(trace_write: dict, pattern_write: list, write_classes: list, trace_read: dict, pattern_read: list,
    ...
    classification_scatter_write = create_categorizer_trace(pattern_write, trace_write['metadata']['start_ts'],
                                                            trace_write['metadata']['end_ts'], 'write')
    ...
    classification_scatter_read = create_categorizer_trace(pattern_read, trace_write['metadata']['start_ts'],
                                                           trace_write['metadata']['end_ts'], 'read')
    ...
    with open(os.path.join(output_dir, trace_write['metadata']['file'] + '.html'), 'w') as f:
...
def create_job_trace(job: dict, operation_type: str, mount: str) -> go.scatter:
    ...
    while access_next_ends:
        ...
        if access_next_starts and next_start['ts'] < next_end['ts'] + next_end['dur']:
            earliest_timestamp = next_start['ts']
            operation = int.__add__
            access_next_starts.pop(0)
        else:
            earliest_timestamp = next_end['ts'] + next_end['dur']
